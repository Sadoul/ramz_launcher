 ...
 *   - ThreadingDetector + AtomicReference (~24 bytes + class metadata)
 *   - 1 instance field per of the above
 *
 * For a typical loaded world ~5_000 chunks * 24 sections * 2 containers (states+biomes)
 * = 240_000 PalettedContainer instances. Cutting 30+ bytes off each ≈ 7 MB heap saved
 * with zero functional change.
 *
 * Compatibility:
 *   - public ctor signatures preserved
 *   - public methods preserved (acquire/release become no-op; the original ones are
 *     advisory thread-conflict detection that already does NOT prevent races, only
 *     reports them in dev mode)
 *   - no field rename — only field REMOVAL of the internal threadingDetector;
 *     it is a private final field, no Forge/coremod public API depends on it
 */
public class PalettedContainer<T> implements PaletteResize<T>, PalettedContainerRO<T> {
   private static final int MIN_PALETTE_BITS = 0;

   /** P-01: shared no-op resize listener — vanilla allocated one lambda per instance. */
   @SuppressWarnings("rawtypes")
   private static final PaletteResize DUMMY_PALETTE_RESIZE = (value, newSize) -> 0;

   @SuppressWarnings("unchecked")
   private static <T> PaletteResize<T> dummyResize() {
      return (PaletteResize<T>) DUMMY_PALETTE_RESIZE;
   }
   ...
   /**
    * P-01: vanilla had `new ThreadingDetector("PalettedContainer")` here, called
    * via acquire()/release() around every read/write. The detector neither prevents
    * races nor synchronises — it is a debug aid that, in modded production builds
    * with hundreds of thousands of containers, only adds memory and CPU overhead.
    * Methods kept for ABI compatibility, bodies emptied.
    */
   public void acquire() {
   ...
   public T getAndSet(int p_63092_, int p_63093_, int p_63094_, T p_63095_) {
      // P-01: acquire/release are no-op now, drop the try/finally overhead too
      return (T) this.getAndSet(this.strategy.getIndex(p_63092_, p_63093_, p_63094_), p_63095_);
   }
   ...
   public PalettedContainerRO.PackedData<T> pack(IdMap<T> p_188065_, PalettedContainer.Strategy p_188066_) {
      // P-01: acquire/release no-op, dropped try/finally
      HashMapPalette<T> hashmappalette = new HashMapPalette<>(p_188065_, this.data.storage.getBits(), PalettedContainer.<T>dummyResize());
      ...
      return new PalettedContainerRO.PackedData<>(hashmappalette.getEntries(), optional);
   }
