 ...
 *
 * Vanilla allocates a full PalettedContainer<BlockState> for every such section
 * regardless. Each empty PalettedContainer carries its own:
 *   - Data record (Configuration + ZeroBitStorage + SingleValuePalette)
 *   - Configuration record
 *   - ZeroBitStorage  (~24 bytes)
 *   - SingleValuePalette (~24 bytes + value ref)
 *
 * Total ~150-200 bytes wasted per empty section.
 *
 * P-02 strategy:
 *   - One JVM-wide shared {@code EMPTY_BLOCK_STATES} container
 *   - {@code states} is no longer final
 ...
 *
 * The biome container CANNOT be shared this way because biome holders are tied to
 * a per-level Registry<Biome>; we leave biomes as-is.
 */
public class LevelChunkSection {
   ...
   /**
    * P-02: shared empty-states container. Held inside a lazy holder so we don't
    * touch {@code Block.BLOCK_STATE_REGISTRY} during {@code LevelChunkSection} class
    * initialisation — the registry is bootstrapped from {@link Blocks} much later
    * during game startup, and an early reference here would either NPE or freeze a
    * partially-initialised IdMap.
    *
    * INVARIANT: instance must never be mutated. All entry points that could mutate
    * (setBlockState, read, getStates) call {@link #promoteStates()} first.
    */
   private static final class EmptyHolder {
      static final PalettedContainer<BlockState> INSTANCE = new PalettedContainer<>(
         Block.BLOCK_STATE_REGISTRY,
         Blocks.AIR.defaultBlockState(),
         PalettedContainer.Strategy.SECTION_STATES);
   }

   private static PalettedContainer<BlockState> sharedEmpty() {
   ...
   private PalettedContainer<BlockState> states; // P-02: was final
   ...
   public LevelChunkSection(Registry<Biome> p_282873_) {
      // P-02: empty section starts with the shared singleton; promotes on first write.
      this.states = sharedEmpty();
   ...
   /** P-02: lazily replaces the shared empty container with a private mutable copy. */
   private void promoteStates() {
      if (this.states == sharedEmpty()) {
   ...
   public BlockState setBlockState(int p_62992_, int p_62993_, int p_62994_, BlockState p_62995_, boolean p_62996_) {
      // P-02: any non-air write must own a private container. Air-air writes on the
      // shared singleton are also unsafe in principle (they touch internal palette
      // bookkeeping), so we promote on every set call.
      this.promoteStates();
   ...
   public PalettedContainer<BlockState> getStates() {
      // P-02: external callers may mutate the returned reference; promote first.
      this.promoteStates();
      return this.states;
   }
   ...
   public void read(FriendlyByteBuf p_63005_) {
      ...
      // P-02: read() rebuilds internal Data — must own a private container.
      this.promoteStates();
