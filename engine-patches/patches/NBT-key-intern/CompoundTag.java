...
public class CompoundTag implements Tag {
    ...
    *
    * We dedup these aggressively at the source: every key coming through
    * readNamedTagName() is funneled through INTERNED_KEYS. Hits return the
    * canonical existing instance; misses are added (bounded to 65535 entries to
    * defend against attacker-supplied keys exhausting heap).
    *
    * Why a custom map instead of String.intern():
    *   - String.intern() goes through the JVM-wide string table which has its own
    *     contention and a ~100ns cost per call due to the synchronized native impl.
   ...
   private static final int INTERN_POOL_HARD_CAP = 65535;

   private static String internKey(String key) {
      if (key == null || key.isEmpty()) return key;
      String existing = INTERNED_KEYS.get(key);
      if (existing != null) return existing;
      // Don't grow unbounded; on overflow just return original (still functional).
      if (INTERNED_KEYS.size() >= INTERN_POOL_HARD_CAP) return key;
      String prev = INTERNED_KEYS.putIfAbsent(key, key);
   ...
   static String readNamedTagName(DataInput p_128433_, NbtAccounter p_128434_) throws IOException {
      // P-NBT: dedup tag keys through INTERNED_KEYS to collapse millions of
      // duplicate "id"/"x"/"y"/"z"/"Damage"/... String allocations.
      return internKey(p_128433_.readUTF());
   }
