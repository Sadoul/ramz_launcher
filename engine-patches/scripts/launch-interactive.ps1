...
# ----------------------------------------------------------------------
# P-LCS: Class loading + JIT acceleration
#
# Why each flag:
#   -Xshare:auto                  use built-in JDK CDS archive automatically
#                                 (~5-10s saved on JDK class loading on cold start)
#   -XX:-DontCompileHugeMethods   compile big methods JIT (Forge generates them)
#   -XX:CICompilerCount=4         4 parallel JIT compiler threads (instead of 2 default)
#                                 -> hot methods get optimized faster -> faster steady state
#   -XX:+TieredCompilation        already on by default but explicit
#   -XX:TieredStopAtLevel=4       full C2 optimization (max speed at the cost of
#                                 ~5s extra warm-up; we want max speed long-term)
#   -XX:ReservedCodeCacheSize=512m big code cache so JIT doesn't evict hot Minecraft
#                                  methods under modded codebase pressure
#   -XX:+UseCodeCacheFlushing
#   -XX:StringTableSize=1000003   bigger interned-String hash table (~30% faster
#                                  string lookup; key for ResourceLocation churn)
# ----------------------------------------------------------------------
# P-LCS lite: only the safe flags. Disabled aggressive ones (-Xshare, TieredStopAtLevel,
# DontCompileHugeMethods) after they triggered Mixin LVT mismatches in mod owo.
